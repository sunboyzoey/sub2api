...
// normalizeOpenAIPassthroughOAuthBody 将透传 OAuth 请求体收敛到与现有 Codex OAuth
// 入口一致的兼容行为：默认补 instructions、抽取 system/developer 到 instructions、
// 清理不支持参数，并统一 compact/store/stream 语义。
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact bool) ([]byte, bool, error) {
	...
	if err := json.Unmarshal(body, &reqBody); err != nil {
		return body, false, fmt.Errorf("normalize passthrough body unmarshal: %w", err)
	}

	isMessagesBridge := isOpenAICompatMessagesBridgeBody(body) || isOpenAICompatMessagesBridgeRequestBody(reqBody)
	opts := codexOAuthTransformOptions{
		IsCompact: compact,
	}
	if isMessagesBridge {
		opts.SkipDefaultInstructions = true
		opts.PreserveDeveloperInput = true
	}
	result := applyCodexOAuthTransformWithOptions(reqBody, opts)
	if !result.Modified {
		return body, false, nil
	}
	...
	if err != nil {
		return body, false, fmt.Errorf("normalize passthrough body marshal: %w", err)
	}
