...
func TestOpenAIGatewayService_OAuthPassthrough_CodexMissingInstructionsNormalizedBeforeUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// Codex OAuth 透传会先执行归一化，缺少 instructions 时会自动补齐后再触达上游。
	...
	require.NotEmpty(t, gjson.GetBytes(upstream.lastBody, "instructions").String())
	require.Equal(t, false, gjson.GetBytes(upstream.lastBody, "store").Bool())
	require.Equal(t, true, gjson.GetBytes(upstream.lastBody, "stream").Bool())
