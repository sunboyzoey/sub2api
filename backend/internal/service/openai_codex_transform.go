...
func applyCodexOAuthTransformWithOptions(reqBody map[string]any, opts codexOAuthTransformOptions) codexTransformResult {
	...
	if extractSystemMessagesFromInputWithOptions(reqBody, !opts.PreserveDeveloperInput) {
		result.Modified = true
	}
...
func extractSystemMessagesFromInput(reqBody map[string]any) bool {
	return extractSystemMessagesFromInputWithOptions(reqBody, true)
}

func extractSystemMessagesFromInputWithOptions(reqBody map[string]any, includeDeveloper bool) bool {
	...
	for _, item := range input {
		...
		role, _ := m["role"].(string)
		if role != "system" && !(includeDeveloper && role == "developer") {
			remaining = append(remaining, item)
			continue
		}
