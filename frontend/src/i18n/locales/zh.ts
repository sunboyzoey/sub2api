export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      dataImportHint: '上传导出的 JSON 文件，或 CPA / Codex OAuth JSON、ZIP 文件以自动转换并批量导入账号。',
      dataImportWarning: '导出的 JSON 可以原样导入；CPA / Codex OAuth JSON、ZIP 会自动转换为现有 OpenAI OAuth 账号格式，并优先继承所选模板账号的默认配置；未指定时则回退到最近一个已分组 OpenAI OAuth 账号。',
      dataImportFile: '数据文件',
      dataImportFileTypes: 'JSON (.json) / ZIP (.zip)',
      dataImportFilesSelected: '已选择 {count} 个文件',
      dataImportDomainFilter: '邮箱域过滤',
      dataImportDomainFilterPlaceholder: '例如：outlook.com, gmail.com',
      dataImportDomainFilterHint: '可选，仅对 CPA / Codex OAuth JSON、ZIP 导入生效；留空表示不过滤。',
      dataImportTemplateAccount: '模板账号',
      dataImportTemplateAccountAuto: '自动选择最近一个已分组 OpenAI OAuth 账号',
      dataImportTemplateAccountHint: '可选，仅对 CPA / Codex OAuth JSON、ZIP 导入生效；指定后会优先继承该账号的分组、并发、优先级、倍率和自动过期暂停配置。',
      ...
      dataImportResultSummary: '代理创建 {proxy_created}，复用 {proxy_reused}，失败 {proxy_failed}；账号创建 {account_created}，跳过已存在 {account_skipped_existing}，失败 {account_failed}',
      dataImportErrors: '失败详情',
      dataImportSuccess: '导入完成：账号 {account_created}，跳过已存在 {account_skipped_existing}，失败 {account_failed}',
