...
const panelClass = computed(() => props.embedded
  ? 'w-full max-w-5xl mx-auto px-6 py-10'
  : 'w-full max-w-5xl mx-auto px-6 py-12')
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.subscription) {
      ...
      for (const limitItem of limits) {
        if (limitItem.limit != null && limitItem.limit > 0) {
          const pct = Math.min(Math.round((limitItem.usage / limitItem.limit) * 100), 100)
          items.push({ title: limitItem.label, pct, amount: `${usd(limitItem.usage)} / ${usd(limitItem.limit)}`, iconType: 'calendar' })
        }
      }
    }
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  const iconShield = '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>'
  const iconCalendar = '<rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>'
  const iconDollar = '<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>'
